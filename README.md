...
If we ignore the current year (incomplete dataset) and focus on the
general trends, we can see that it does look like the number of
incidents has been going down over time. However, there seems to be some
sharp downturns that could be due to missing data.
...
-   We can see that average injuries for the most common events are
    relatively low, while the one accident attributed to the Main System
    caused massive numbers of injuries.
